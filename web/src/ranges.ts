...
const RFI: Record<string, string> = {
  EP: "22+, ATs+, A5s-A4s, KTs+, QTs+, JTs, T9s, 98s, 87s, 76s, 65s, AJo+, KQo",
  HJ: "22+, A9s+, A5s-A2s, K9s+, Q9s+, J9s+, T9s, 98s, 87s, 76s, 65s, 54s, ATo+, KJo+, QJo",
  CO: "22+, A2s+, K7s+, Q8s+, J8s+, T8s+, 97s+, 87s, 76s, 65s, 54s, A9o+, A5o, KTo+, QTo+, JTo",
  BTN: "22+, A2s+, K2s+, Q4s+, J6s+, T6s+, 95s+, 85s+, 74s+, 64s+, 53s+, 43s, A4o+, K9o+, Q9o+, J9o+, T9o, 98o",
  SB: "22+, A2s+, K2s+, Q3s+, J5s+, T6s+, 96s+, 85s+, 75s+, 64s+, 54s, A2o+, K8o+, Q9o+, J9o+, T9o",
};

// BB flat vs an open, keyed by opener
const BB_FLAT: Record<string, string> = {
  EP: "22-99, A2s-AJs:0.75, K8s+, Q9s+, J8s+, T8s+, 97s+, 87s, 76s, 65s, 54s, ATo:0.5, AJo:0.75, KJo+, QJo",
  HJ: "22-99, A2s-AJs:0.75, K7s+, Q8s+, J8s+, T8s+, 97s+, 87s, 76s, 65s, 54s, ATo:0.75, AJo, KJo+, QJo, JTo:0.5",
  CO: "22-99, A2s-AJs, K5s+, Q7s+, J7s+, T7s+, 96s+, 86s+, 75s+, 64s+, 54s, A5o-A9o:0.5, ATo, KTo+, QTo+, JTo, T9o",
  BTN: "22-99, A2s-A9s, ATs:0.5, K2s+, Q2s+, J4s+, T6s+, 96s+, 85s+, 74s+, 63s+, 53s+, 43s, A2o-A9o, K9o+, K8o:0.5, Q9o+, J9o+, T8o+, 97o+, 87o, 76o:0.5",
  SB: "22+, A2s+, K2s+, Q2s+, J2s+, T4s+, 95s+, 84s+, 74s+, 63s+, 52s+, 42s+, 32s, A2o+, K4o+, Q6o+, J7o+, T7o+, 97o+, 86o+, 76o, 65o",
};

// SB flat vs an open (tight, no dead-money incentive)
const SB_FLAT =
...
const IP_FLAT =
  "22-JJ, QQ:0.25, AQs:0.5, AJs, ATs:0.5, KQs, KJs:0.5, QJs, JTs, T9s, 98s:0.5, 87s:0.5, AQo:0.3";

// 3bet ranges
const BB_3BET: Record<string, string> = {
  EP: "QQ+, JJ:0.5, AKs, AQs:0.5, A5s-A4s:0.5, KQs:0.3, AKo, AQo:0.3",
  HJ: "JJ+, TT:0.5, AQs+, AJs:0.3, A5s-A4s:0.6, KQs:0.5, KJs:0.3, AKo, AQo:0.5, KQo:0.3",
  CO: "TT+, 99:0.3, AJs+, ATs:0.3, A5s-A2s:0.5, KQs, KJs:0.5, QJs:0.3, JTs:0.3, AQo+, ATo:0.2, KQo:0.5",
  BTN: "TT+, 99:0.3, ATs+, A5s-A2s:0.6, K9s+, QTs+, JTs, T9s:0.5, 98s:0.5, 76s:0.3, 65s:0.3, AJo+, ATo:0.3, KQo, KJo:0.5, QJo:0.3",
  SB: "99+, A9s+, A5s-A2s:0.6, K9s+, QTs+, JTs, T9s:0.5, 98s:0.5, AJo+, ATo:0.4, KQo, KJo:0.5",
};

const SB_3BET: Record<string, string> = {
  EP: "QQ+, JJ:0.6, AKs, AQs:0.6, A5s:0.5, KQs:0.4, AKo, AQo:0.3",
  HJ: "JJ+, TT:0.6, AQs+, AJs:0.5, A5s-A4s:0.6, KQs:0.6, AKo, AQo:0.6",
  CO: "TT+, 99:0.5, AJs+, ATs:0.5, A5s-A4s, KQs, KJs:0.5, QJs:0.4, AQo+, KQo:0.4",
  BTN: "99+, 88:0.5, ATs+, A5s-A2s:0.7, K9s+, QTs+, JTs, T9s:0.4, AJo+, ATo:0.4, KQo, KJo:0.4",
};

const IP_3BET: Record<string, string> = {
  EP: "JJ+, TT:0.3, AQs+, AJs:0.3, A5s:0.5, KQs:0.5, AKo, AQo:0.3",
  HJ: "TT+, 99:0.3, AQs+, AJs:0.5, A5s-A4s:0.5, KQs:0.7, KJs:0.3, AKo, AQo:0.5, KQo:0.2",
  CO: "99+, 88:0.5, AJs+, ATs:0.5, A5s-A4s, KQs, KJs:0.5, QJs:0.5, JTs:0.5, T9s:0.5, AQo+, AJo:0.3, KQo:0.3",
  BTN: "", // BTN never faces a later non-blind 3bettor
};

// opener continuing vs a 3bet (call), keyed by whether the 3bettor is a blind
// (opener in position) or not (opener out of position)
const CALL_3BET_OOP =
  "99-QQ, KK:0.4, AA:0.4, 88:0.5, AQs+, AJs:0.5, ATs:0.4, A5s:0.5, KQs, KJs:0.4, QJs:0.5, JTs:0.5, T9s:0.5, 98s:0.4, AQo:0.5, AKo:0.6";
const CALL_3BET_IP =
  "22-JJ, QQ:0.5, KK:0.25, AA:0.25, A8s+, A5s-A2s:0.5, K9s+, Q9s+, J9s+, T8s+, 97s+, 87s, 76s, 65s, 54s, AJo+, KQo, KJo:0.5";

// 4bet pots
const FOURBET_RANGE = "KK+, QQ:0.6, JJ:0.2, AKs, AKo:0.7, A5s:0.4, AQs:0.2";
const CALL_4BET =
...
function flatVsOpen(defender: Position, opener: Position): Float32Array {
  if (defender === "BB") return parseRange(BB_FLAT[opener]);
  if (defender === "SB") return parseRange(SB_FLAT);
  return parseRange(IP_FLAT);
}

function threeBetVsOpen(defender: Position, opener: Position): Float32Array {
  if (defender === "BB") return parseRange(BB_3BET[opener]);
  if (defender === "SB") return parseRange(SB_3BET[opener] ?? SB_3BET.CO);
  return parseRange(IP_3BET[opener] ?? IP_3BET.CO);
}
...
export function buildScenario(
  ...
  const rfi = parseRange(RFI[opener]);
  const openerInPosition =
    POSTFLOP_ORDER.indexOf(opener) > POSTFLOP_ORDER.indexOf(defender);
  if (potType === "srp") {
    openerRange = rfi;
    defenderRange = flatVsOpen(defender, opener);
    committed = openTo;
    description = `${opener} ${openTo}bb オープン → ${defender} コール`;
  } else if (potType === "3bp") {
    openerRange = intersect(
      parseRange(openerInPosition ? CALL_3BET_IP : CALL_3BET_OOP),
      rfi
    );
    defenderRange = threeBetVsOpen(defender, opener);
    committed = threeBetTo;
    description = `${opener} オープン → ${defender} ${threeBetTo}bb 3bet → ${opener} コール`;
  } else {
    openerRange = intersect(parseRange(FOURBET_RANGE), rfi);
    defenderRange = intersect(
      parseRange(CALL_4BET),
      threeBetVsOpen(defender, opener)
    );
    committed = fourBetTo;
